...
class PipelineFactory:
    ...
    that also retains feature names. As of version 1.1.2 for sci-kit learn,
    pipelines still cannot retain feature names after transformation, hence
    the customized pipelines produced by this class add a function transformer 
    to retain feature names.
    """
    ...
    def create_pipe(self, *, impute:bool, normalize:bool)->customPipe:
        ...
        encoded in any of the pipelines. All pipelines created will one-hot
        ...
        if impute & normalize:
            return self.make_impute_ohe_scale_pipe(self.cat_cols,self.num_cols)
        elif impute & ~normalize:
            return self.make_impute_ohe_pipe(self.cat_cols,self.num_cols)
        elif ~impute & ~normalize:
            return self.make_ohe_pipe(self.cat_cols,self.num_cols)
        else:
    ...
    def make_impute_ohe_scale_pipe(self)->customPipe:
        ...
        pipe = customPipe([
            ...
            (
                'columnTransform',
                ColumnTransformer([
                    ('categorical_vars',OneHotEncoder(),self.cat_cols),
